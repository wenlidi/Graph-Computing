...
#BUILD_MARK ?=dynamic
#BUILD_MARK ?=lowthreadnew
#BUILD_MARK ?=sort
#BUILD_MARK ?=dy_sort
#BUILD_MARK ?=dy_sort
#BUILD_MARK ?=sort_low
#BUILD_MARK ?=sort_dy
BUILD_MARK ?=coalesced_common

# -------------------------------------------------- General build ----------------------------------------------------------
#USER_PATH ?= bfsdy
#USER_PATH ?= shortest_path_dy
#USER_FILE ?= shortest_path_dy/adjustable_heap.h
#USER_PATH ?= shortest_path
#USER_FILE ?= shortest_path/adjustable_heap.h
#USER_PATH ?= breadth_first_search
#USER_PATH ?= pagerank_dy
USER_PATH ?= page_rank
#USER_PATH ?= wcc
#USER_PATH ?= wcc_dy
#USER_PATH ?= breadth_first_search
...
NVCC_FLAGS := -DLAMBDA_PROFILING=profile -DLAMBDA_SORT_VERTEX_BY_IN_EDGE_COUNT=true -DLAMBDA_IN_EDGE_COALESCED_MEMORY_ACCESS=true -m64 -O3 -rdc=true $(GENCODE_FLAGS) $(GENERAL_INCLUDES) $(GENERAL_LIBRARIES) $(GPREGEL_INCLUDES)
NVCC_FLAGS_SORT := -DLAMBDA_PROFILING=profile -DLAMBDA_SORT_VERTEX_BY_IN_EDGE_COUNT=true -DLAMBDA_IN_EDGE_COALESCED_MEMORY_ACCESS=true -m64 -O3 -rdc=true $(GENCODE_FLAGS) $(GENERAL_INCLUDES) $(GENERAL_LIBRARIES) $(GPREGEL_INCLUDES)
...
builddy: prepare $(OBJSDY) $(MAIN_OBJ)
	$(NVCC) -lpthread $(NVCC_FLAGS_SORT) -o $(OUT_DIR)/maindy_args $(OBJSDY) $(MAIN_OBJ) /usr/lib/x86_64-linux-gnu/libgflags.a

buildall: prepare $(OBJSDY) $(MAIN_OBJ)
	$(NVCC) -lpthread $(NVCC_FLAGS_SORT) -o $(OUT_DIR)/maindy_args $(OBJSDY) $(MAIN_OBJ) /usr/lib/x86_64-linux-gnu/libgflags.a

buildlow: prepare $(OBJSLOW) $(MAIN_OBJ)
	$(NVCC) -lpthread $(NVCC_FLAGS_SORT) -o $(OUT_DIR)/mainlow $(OBJSLOW) $(MAIN_OBJ) /usr/lib/x86_64-linux-gnu/libgflags.a

buildsort: prepare $(OBJS) $(MAIN_OBJ)
	$(NVCC) -lpthread $(NVCC_FLAGS_SORT) -o $(OUT_DIR)/mainsort $(OBJS) $(MAIN_OBJ) /usr/lib/x86_64-linux-gnu/libgflags.a

buildsortdy: prepare $(OBJSDY) $(MAIN_OBJ)
	$(NVCC) -lpthread $(NVCC_FLAGS_SORT) -o $(OUT_DIR)/mainsortdy $(OBJSDY) $(MAIN_OBJ) /usr/lib/x86_64-linux-gnu/libgflags.a
...
$(OBJ_DIR)/%.o: $(CORE_DIR)/%.cc
	$(NVCC) $(NVCC_FLAGS_SORT) -c $< -o $(OBJ_DIR)/$*.o
