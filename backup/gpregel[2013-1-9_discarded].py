...
#
# Additional attributes for in members:
# map['rand_val'] = 'RandUtil::'FuncName or user defined compile-time constants
#
# Additional attributes for out members:
# map['init_val'] = user defined compile-time constants
#
# Additional attributes for out-diff members:
# map['diff'] = 0 (for all kinds of types) or some compile-time float constants (for float numbers)
...
pattern_str_in = 'in'
pattern_str_out = 'out'
pattern_str_type = r'int|unsigned\s*int|char|bool|float'
pattern_str_name = r'[_a-zA-Z]\w*'
pattern_str_val = r'[^;]+'

def GetMatchStatements(content, name, str_statement, matched_str_statement):
  str_struct = r'(?<!\w)struct\s+' + name + r'\s*\{(?:\s*' + str_statement + r')*\s*\}'
  ...
  pat_matched_statement = re.compile(matched_str_statement)
  print('matched statement pattern: ', matched_str_statement)

  matches = []
  for mem_str in def_member:
    match = re.findall(pat_matched_statement, mem_str)
    matches.append(match)

  print(matches)
  return matches

def GetGlobalMap(content):
  str_statement =
      r'(?:' + pattern_str_type + r')\s+'
      + pattern_str_name + r'\s*'
      + r'(?:=\s*' + pattern_str_val + r')?;'
  matched_str_statement =
      r'(' + pattern_str_type + r')\s+'
      + r'(' + pattern_str_name + r')\s*'
      + r'(?:=\s*(' + pattern_str_val + r'))?;'
  matches = GetMatchStatements(content, 'Global', str_statement, matched_str_statement)
