...
DEFINE_int32(dy_fenshuiling, 50064, "");
DEFINE_int32(vertice_divide, 1, "");
DEFINE_int32(blocksize_inner, 128, "");
DEFINE_string(input_file, "", "");
DEFINE_string(sort_type, "id", "");
...
//  cout << "FLAGS_num_gpu:=" << FLAGS_num_gpus;
//  cout << "FLAGS_max_superstep:=" << FLAGS_max_spuerstep;
//  cout << "FLAGS_dy_fenshuiling:=" << FLAGS_dy_fenshuiling;
  conf->SetNumGPUControlThreads(FLAGS_num_gpus);
  conf->SetMaxSuperstep(FLAGS_max_superstep);
  conf->SetFenShuiLing(FLAGS_dy_fenshuiling);
  conf->SetVerticeDivide(FLAGS_vertice_divide);
   conf->SetBlocksizeInner(FLAGS_blocksize_inner);
   conf->SetSortType(FLAGS_sort_type);
