...
//for sssp
/*
__global__ void K_UserCompute(int fenshuiling) {
...
//		printf("1500 use dyp\n");
	   int grid =(out_edge_Num+128-1)/128 ;
           sendmessagesssp<<<grid,128>>>(it,message,out_edge_Num);
           }else{
...
/*
// for bfs
__global__ void sendmessagebfs(OutEdgeIterator it){
   int idx = blockDim.x*blockIdx.x+threadIdx.x;
   it.AddOffset(idx);
   Message msg(*it);
      msg.Send();
...
*/
// for pagerank
__global__ void sendmessagepagerank(OutEdgeIterator it,float averaged){
   int idx = blockDim.x*blockIdx.x+threadIdx.x;
   it.AddOffset(idx);
   Message msg(*it);
...
__global__ void K_UserCompute(int fenshuiling) {
  ...
  for (unsigned int idx = tid; idx < num_vertex; idx += num_threads) {
    ...
    OutEdgeIterator it = v.GetOutEdgeIterator();         
          if(out_edge_num > fenshuiling){
            int grid =(out_edge_num+128-1)/128 ;
            sendmessagepagerank<<<grid,128>>>(it,message);
          }else{
           for (; !it.Done(); it.Next()) {
           Message msg(*it);
...
//cout << "fenshuiling is :"<<fenshuiling<<endl;
  K_UserCompute<<<num_blocks, num_threads_per_block>>>(fenshuiling);
