...
/*
__global__ void sendmessage(OutEdgeIterator it,int mindist){
   int idx = threadIdx.x;
   it.AddOffset(idx);
   Message msg(*it);
      msg.Send();
...
__global__ void K_UserComputeSort(int size) {
  const unsigned int num_vertex = d_vcon.d_size;
  GET_NUM_THREAD_TID_AND_RETURN_IF_TID_GE(num_vertex);
  int up =(tid+1)*size;
  for (unsigned int idx = tid* size; (idx<up)&& (idx< num_vertex); idx++ ) {
    Vertex v(idx);
    MessageIterator msgs(idx);
    v.Compute(&msgs);
  }
