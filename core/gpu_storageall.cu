...
struct SortVconByInEdgeCount_LT_ASC {
  __device__ bool operator()(const unsigned int idx1, const unsigned int idx2) {
    return d_vcon.d_in_edge_count[idx1] == d_vcon.d_in_edge_count[idx2]
        ? d_vcon.d_out_edge_count[idx1] < d_vcon.d_out_edge_count[idx2]
        : d_vcon.d_in_edge_count[idx1] < d_vcon.d_in_edge_count[idx2];
...
struct SortVconByOutEdgeCount_LT {
  __device__ bool operator()(const unsigned int idx1, const unsigned int idx2) {
    return d_vcon.d_out_edge_count[idx1] == d_vcon.d_out_edge_count[idx2]
        ? d_vcon.d_in_edge_count[idx1] > d_vcon.d_in_edge_count[idx2]
        : d_vcon.d_out_edge_count[idx1] > d_vcon.d_out_edge_count[idx2];
  }
};
...
struct SortVconByOutEdgeCount_LT_ASC {
  __device__ bool operator()(const unsigned int idx1, const unsigned int idx2) {
    return d_vcon.d_out_edge_count[idx1] == d_vcon.d_out_edge_count[idx2]
        ? d_vcon.d_in_edge_count[idx1] < d_vcon.d_in_edge_count[idx2]
        : d_vcon.d_out_edge_count[idx1] < d_vcon.d_out_edge_count[idx2];
...
__global__ void sendmessagesssp(OutEdgeIterator it,int mindist,int edgenum){
...
__global__ void K_UserCompute(int fenshuiling,int blocksize_inner) {
  ...
  for (unsigned int idx = tid; idx < num_vertex; idx += num_threads) {
    ...
    if(message == -1){
	   ...
	   OutEdgeIterator it = v.GetOutEdgeIterator();
           if(out_edge_Num>fenshuiling ){
//		printf("1500 use dyp\n");
	   int grid =(out_edge_Num+blocksize_inner-1)/blocksize_inner ;
           sendmessagesssp<<<grid,blocksize_inner>>>(it,message,out_edge_Num);
           }else{
...
/*
// for bfs
__global__ void sendmessagebfs(OutEdgeIterator it,int outedgenum){
   int idx = blockDim.x*blockIdx.x+threadIdx.x;
   if(idx<outedgenum){
      it.AddOffset(idx);
      Message msg(*it);
  ...
  for (unsigned int idx = tid; idx < num_vertex; idx += num_threads) {
    ...
    if(message == false){
       return;
          ...
          if(out_edge_num > fenshuiling){
	   int grid =(out_edge_num+blocksize_inner-1)/blocksize_inner ;
           sendmessagebfs<<<grid,blocksize_inner>>>(it,out_edge_num);
          }else{
           for (; !it.Done(); it.Next()) {
           Message msg(*it);
           msg.Send();
...
/*
// for pagerank
__global__ void sendmessagepagerank(OutEdgeIterator it,float averaged,int outedgenum){
   int idx = blockDim.x*blockIdx.x+threadIdx.x;
   if(idx<outedgenum){
   it.AddOffset(idx);
   Message msg(*it);
...
__global__ void K_UserCompute(int fenshuiling,int blocksize_inner) {
  ...
  for (unsigned int idx = tid; idx < num_vertex; idx += num_threads) {
    Vertex v(idx);
    MessageIterator msgs(idx);
    float message = v.Compute(&msgs);
  
    int out_edge_num = v.get_out_edge_count() ;  
    OutEdgeIterator it = v.GetOutEdgeIterator();         
          if(out_edge_num > fenshuiling){
            int grid =(out_edge_num+blocksize_inner-1)/blocksize_inner ;
            sendmessagepagerank<<<grid,blocksize_inner>>>(it,message,out_edge_num);
          }else{
           for (; !it.Done(); it.Next()) {
           Message msg(*it);
           msg.set_rank(message);
           msg.Send();
         }
      }
    }
...
__global__ void sendmessagewcc(OutEdgeIterator it,int curlevel,int edgenum){
   int idx = blockDim.x*blockIdx.x+threadIdx.x;
  if(idx<edgenum){
       it.AddOffset(idx);
//     printf("inner sendmessate idx=%d\n",idx);
      Message msg(*it);
      msg.set_level(curlevel);
      msg.Send();
   }
...
//    printf("1500 use dyp\n");
     int grid =(out_edge_Num+blocksize_inner-1)/blocksize_inner ;
           sendmessagewcc<<<grid,blocksize_inner>>>(it,message,out_edge_Num);
           }else{
           for (it = v.GetOutEdgeIterator(); !it.Done(); it.Next()) {
           Message msg(*it);
            msg.set_level(message);
           msg.Send();
         }
      }
    }
...
void GPUStorageManager::UserCompute(VertexContent *vcon) {
  ...
  static unsigned int divide = conf->GetVerticeDivide();
//cout <<"vertice divide to "<<divide <<endl;
  static unsigned int num_blocks =
      (vcon->d_size/divide + num_threads_per_block - 1) / num_threads_per_block; 
#ifdef LAMBDA_DEBUG
  DBG_WRAP_COUT(
...
#endif
static unsigned int fenshuiling = conf->GetFenShuiLing();
static unsigned int blocksize_inner = conf->GetBlocksizeInner();
//cout << "fenshuiling is :"<<fenshuiling<<endl;
  K_UserCompute<<<num_blocks, num_threads_per_block>>>(fenshuiling,blocksize_inner);
...
void GPUStorageManager::SingleGPUBuildVconIndexes(VertexContent *vcon) {
  ...
  string type = conf->getSortType();
  if(type ==value("in_desc") ){
  cout << "sort by in_edge_count desc" <<endl;
  thrust::sort(thr_shuffle_index, thr_shuffle_index + vcon->d_size, SortVconByInEdgeCount_LT());
  }
  if(type ==value("in_asc") ){
  cout << "sort by in_edge_count asc" <<endl;
  thrust::sort(thr_shuffle_index, thr_shuffle_index + vcon->d_size, SortVconByInEdgeCount_LT_ASC());
  }
  if(type ==value("out_desc") ){
  cout << "sort by out_edge_count desc" <<endl;
  thrust::sort(thr_shuffle_index, thr_shuffle_index + vcon->d_size, SortVconByOutEdgeCount_LT());
  }
  if(type ==value("out_asc") ){
  cout << "sort by out_edge_count asc" <<endl;
  thrust::sort(thr_shuffle_index, thr_shuffle_index + vcon->d_size, SortVconByInEdgeCount_LT_ASC());
  }
  if(type ==value("id") ){
